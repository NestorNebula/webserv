...
/*   Updated: 2026/09/10 14:52:18 by kdonlon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	FcgiPipe::timeo(WsTime & now)
{
	...
	if (this->rsrc)
	{
		...
		// if (rsrc->done & RSRC_DONE_IP)
		// {
		// 	WSCOL(WSL_GREEN);
		// 	WSLOG(LVL_DBG, TGT_FCGI | TGT_TIMEO, "(ip)");
		// 	return (false);
		// }
...
int		FcgiPipe::rdhup(void)
{
	...
	{
		WSCOL(WSL_YELLOW);
		WSLOG(LVL_DBG, TGT_FCGI, "rdhup: resp.size() ", this->rsrc->get_resp().size());
